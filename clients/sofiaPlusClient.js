...
export class SofiaPlusClient {
  constructor({ headless, slowMo, retries, retryDelay, logger } = {}) {
    this.headless =
  ...
  async #initializeBrowser() {
    ...
    try {
      ...
      this.logger?.info('Navegador', `Iniciando Chromium (Headless: ${this.headless}, Plataforma: ${process.platform})...`);
      ...
      this.logger?.log('Navegador', 'Navegador Chromium listo y configurado con scripts anti-detección.');
      return this.page;
    } catch (error) {
      this.browser = null;
      this.page = null;
      ...
      this.logger?.error('Navegador', 'Error al iniciar navegador Chromium', error);
  ...
  async #loginOnce() {
    ...
    this.logger?.info('Portal SOFIA', `Navegando a ${this.sofiaUrl}...`);
    ...
    if (pageText.includes('verificación') || pageText.includes('captcha') || pageText.includes('Access denied')) {
      this.logger?.error('Seguridad Anti-bot', 'SOFIA Plus mostró página de verificación/captcha');
    ...
    this.logger?.info('Formulario Login', 'Buscando iframe de autenticación (#registradoBox1)...');
    await page.waitForSelector('#registradoBox1', { timeout });

    const iframeHandle = await page.$('#registradoBox1');
    if (!iframeHandle) {
      this.logger?.error('Formulario Login', 'No se encontró iframe #registradoBox1');
      throw new Error(
    ...
    this.logger?.info('Credenciales', `Ingresando documento de identidad (${sofiaUser})...`);
    await loginFrame.getByRole('textbox', { name: 'Número de Documento' }).fill(sofiaUser);
    await loginFrame.getByRole('textbox', { name: 'Contraseña' }).fill(sofiaPass);
    this.logger?.info('Autenticación', 'Haciendo clic en "Ingresar" y esperando selector de rol...');
    ...
    try {
      ...
      this.logger?.log('Autenticación Exitosa', 'Credenciales aceptadas correctamente por SOFIA Plus.');
    } catch (timeoutError) {
      ...
      try {
        ...
        if (errorEl) {
          ...
          this.logger?.error('Rechazo Credenciales', `SOFIA Plus rechazó las credenciales: ${text.trim()}`);
          throw new SofiaFatalError(
      ...
      this.logger?.error('Timeout Autenticación', 'No apareció el selector de roles tras 60s', timeoutError);
    ...
    await withRetry(
      ...
      },
      {
        ...
        onRetry: async ({ attempt }) => {
          ...
          this.logger?.warn('Reintento Login', `Reiniciando navegador para intento ${attempt + 1}/${this.retries + 1}...`);
  ...
  async selectRole(roleLabel = 'Gestión Desarrollo Curricular') {
    ...
    try {
      this.logger?.info('Selección Rol', `Seleccionando rol "${roleLabel}"...`);
      ...
      this.logger?.log('Rol Seleccionado', `Rol "${roleLabel}" confirmado exitosamente.`);
    } catch (error) {
      this.logger?.error('Selección Rol', `Fallo al seleccionar rol "${roleLabel}"`, error);
