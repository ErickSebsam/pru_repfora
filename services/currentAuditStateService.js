 ...
 * @param {Array} newsExcluded - Aprendices excluidos por novedad (opcional)
 */
export async function updateCurrentAuditState(pendientes = [], vencidos = [], fichesSummary = [], newsExcluded = []) {
  ...
  console.log(`[AUDIT_STATE] Ejecución guardada. Total runs: ${doc.runs.length}/${MAX_RUNS}. Pendientes: ${pendientes.length}, Vencidos: ${vencidos.length}, Fichas resumidas: ${fichesSummary.length}, Excluidos novedad: ${newsExcluded.length}`);
...
export async function getCurrentAuditState() {
  ...
  if (!doc || !doc.runs.length) {
    return {
      ...
      estadisticas: { totalPendientes: 0, totalVencidos: 0, totalAprendicesSinNota: 0, totalExcluidosNovedad: 0 }
    };
  }
