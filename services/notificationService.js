...
/**
 * Envía una notificación consolidada en formato tabla al coordinador con el resumen
 * de aprendices con juicios evaluativos pendientes por instructor y ficha (ejecución DF-14).
 * @param {Array<{fichaNumber: string, enFormacion?: number, pendientes?: number, instructorName?: string}>} records - Registros filtrados del DF-14A.
 * @param {string} coordinationName - Nombre de la coordinación.
 * @returns {Promise<{ success: boolean, count?: number, error?: string }>}
 */
export async function sendCoordinatorMissingJudgmentsNotification(records, coordinationName = 'PROGRAMAS ESPECIALES') {
  if (!emailEnabled) {
    console.log('[EMAIL] Envío de correos deshabilitado (emailEnabled=false)');
  ...
  // Buscar coordinación para obtener credenciales y email del coordinador
  let coordination = null;
  try {
    const Coordination = (await import('../models/Coordination.js')).default;
    coordination = await Coordination.findOne({ name: coordinationName, status: 0 })
    ...
    console.warn('[EMAIL] No se pudo buscar coordinación para reporte de coordinador:', err.message);
  }
  ...
  if (!fromEmail || !fromPass) {
    console.error('[EMAIL] Sin credenciales de correo para enviar notificación al coordinador');
  ...
  for (const record of safeRecords) {
    let instructorName = record.instructorName;
    const fichaNumber = record.fichaNumber;
    const enFormacion = record.enFormacion || record.pendientes || 0;

    if (!instructorName) {
      try {
        const fiche = await Fiche.findOne({ number: fichaNumber, status: 0 })
          .populate('owner', 'name email');
        if (fiche && fiche.owner) {
          instructorName = fiche.owner.name;
        } else {
          instructorName = 'No asignado';
        }
      } catch (err) {
        instructorName = 'No asignado';
      }
  ...
  if (USE_TEST_RECIPIENT) {
    toEmails = [TEST_MAIL_RECIPIENT];
    console.log(`[EMAIL] (TEST MODE: Notificación juicios coordinador redirigido a ${TEST_MAIL_RECIPIENT})`);
  } else {
    const coordEmail = coordination?.coordinator?.email || coordination?.email;
    if (coordEmail) {
      toEmails.push(coordEmail);
    }
    if (coordination?.emailsupervisor) {
      ccEmails = coordination.emailsupervisor.split(',').map(e => e.trim()).filter(Boolean);
    }
  }

  if (toEmails.length === 0) {
    console.warn(`[EMAIL] Sin correo de coordinador para ${coordinationName}, omitiendo envío`);
    return { success: false, error: 'Sin correo de coordinador' };
  }

  try {
    const payload = {
      coordinationName: coordination?.name || coordinationName,
      items
    };

    let subject = items.length > 0
      ? `Resumen Juicios Pendientes DF-14 - ${coordination?.name || coordinationName}`
      : `Resumen DF-14: Sin Juicios Pendientes - ${coordination?.name || coordinationName}`;
    let htmlContent = null;

    try {
      const ComplementaryEmailTemplate = (await import('../models/ComplementaryEmailTemplate.js')).default;
      const tpl = await ComplementaryEmailTemplate.findOne({ key: 'juiciosPendientesCoordinador', status: 0 });
      if (tpl && tpl.htmlContent) {
        const handlebars = (await import('handlebars')).default;
        subject = handlebars.compile(tpl.subject || subject)(payload);
        htmlContent = handlebars.compile(tpl.htmlContent)(payload);
      }
    } catch (tplErr) {
      console.warn('[EMAIL] Error cargando template dinámico, usando fallback .hbs:', tplErr.message);
    }
    ...
    if (htmlContent) {
      ...
      await sendEmail(
        ...
        subject,
        payload,
        './template/resumenJuiciosCoordinador.hbs',
        ccEmails.length > 0 ? ccEmails : null,
        null
      );
    }

    console.log(`[EMAIL] ✓ Notificación juicios coordinador enviada a ${toEmails.join(', ')} (${items.length} fichas)`);
    return { success: true, count: items.length };
  } catch (error) {
    console.error(`[EMAIL] ✗ Error enviando notificación de juicios al coordinador:`, error.message);
    return { success: false, error: error.message };
  }
}
 ...
 */
export async function sendCoordinatorReport(coordination, fichas, fichesSummary = [], pendientes = [], vencidos = [], newsExcluded = []) {
  // Validaciones
  if (!emailEnabled) {
    console.log('[EMAIL] Envío de correos deshabilitado (emailEnabled=false)');
    return { success: false, error: 'Email disabled' };
...
/**
 * Clase recolectora de logs cronológicos y eventos durante la ejecución del DF-14A.
 */
export class ExecutionLogger {
  ...
  log(step, message, status = 'success', details = null) {
    const now = new Date();
    ...
    const entry = { step, message, status, details, time, elapsedMs };
    this.logs.push(entry);
    console.log(`[DF14A_LOG] [${time}] [${status.toUpperCase()}] ${step}: ${message}`);
    return entry;
  }

  info(step, message, details = null) {
    return this.log(step, message, 'info', details);
  }

  warn(step, message, details = null) {
    return this.log(step, message, 'warning', details);
  }

  error(step, message, err = null) {
    const errorDetails = err?.stack || err?.message || (typeof err === 'string' ? err : null);
    return this.log(step, message, 'error', errorDetails);
  }
