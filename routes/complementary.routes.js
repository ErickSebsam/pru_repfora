...
import { compCtrl, getDf14aCronStatusController, toggleDf14aCronController } from "../controller/complementary.controller.js";
...
const {
  ...
  validateGetEmailTemplateByKey,
  validateUpdateEmailTemplate,
  validateResetEmailTemplate,
  // Preferencias de notificación
  validateGetNotificationPrefs,
  validateUpdateNotificationPrefs,
} = complementaryVali;

const { validateAuditDF14, validateAuditDF14Test, validateAuditDF14Status, validateAdminUpdateRequest } = complementaryVali;
...
/**
 * @swagger
 * /api/complementary/catalog/crud:
 *   get:
 *     summary: Obtiene todos los cursos del catalogo (incluidos inhabilitados) para el CRUD
 *     tags: [Complementarias]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Lista de cursos del catalogo para administración
 */
routerComplementary.get("/catalog/crud", validateAdminCatalogQuery, getAdminCatalogs);
 ...
 * /api/complementary/catalog:
 *   post:
 *     summary: Crea un nuevo curso en el catálogo de complementarias
 *     tags: [Complementarias]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *     responses:
 *       200:
 *         description: Programa agregado al catálogo
 *       400:
 *         description: Error de validación o el código ya existe
 */
routerComplementary.post("/catalog", validateCreateCatalog, createCatalog);

/**
 * @swagger
 * /api/complementary/catalog/{id}:
 *   put:
 *     summary: Actualiza los datos de un curso del catálogo por ID
 *     tags: [Complementarias]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         description: Error de validación o el código ya existe
 */
routerComplementary.put("/catalog/:id", validateUpdateCatalog, updateCatalog);

/**
 * @swagger
 * /api/complementary/catalog/{id}/status:
 *   patch:
 *     summary: Habilita o inhabilita un curso del catálogo (alterna el estado)
 *     tags: [Complementarias]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Estado del programa cambiado correctamente
 */
routerComplementary.patch("/catalog/:id/status", validateToggleCatalogStatus, toggleCatalogStatus);
...
/**
 * @swagger
 * /api/complementary/requests/next-consecutive:
 *   get:
 *     summary: Obtiene el próximo número consecutivo disponible para una solicitud
 *     tags: [Complementarias]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: Número consecutivo obtenido correctamente
 */
routerComplementary.get("/requests/next-consecutive", validateHeaders, getNextConsecutive);
...
const parseMultipartArrays = (req, res, next) => {
  const fieldsToParse = ["instructores", "competencies", "outcomes", "sesiones"];
...
routerComplementary.post("/requests/register", parseMultipartArrays, validateRegisterRequest, registerRequest);
...
/**
 * @swagger
 * /api/complementary/requests/{id}/admin:
 *   put:
 *     summary: Edición generalizada de una solicitud por parte de un administrador, coordinador o programador
 *     tags: [Complementarias]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *               actaTrazabilidad:
 *                 type: string
 *                 description: "Número de acta de trazabilidad (opcional)"
 *     responses:
 *       200:
 *         description: Solicitud actualizada correctamente por el administrador
 *       400:
 *         description: Error de validación o la solicitud no existe
 *       403:
 *         description: Sin permisos
 */
routerComplementary.put("/requests/:id/admin", validateAdminUpdateRequest, adminUpdateRequest);
 ...
 *               environment:
 *                 type: string
 *                 description: "ID del ambiente de formación (select, opcional)"
 *               ambienteNombre:
 *                 type: string
 *                 description: "Nombre del ambiente personalizado - Otro (opcional)"
 *     responses:
 *       200:
 *         description: Ficha asignada correctamente
 *       400:
 *         description: Solicitud no existe o no está en APROBADA o PROGRAMADA
 *       403:
 *         description: Sin permisos
 */
...
routerComplementary.get("/reports/df14a/cron-status", getDf14aCronStatusController);
routerComplementary.post("/reports/df14a/cron-toggle", toggleDf14aCronController);
...
/**
 * @swagger
 * /api/complementary/email-templates:
 *   get:
 *     summary: Lista todos los templates de correo de complementarias
 *     description: Devuelve metadata de todos los templates (sin el HTML completo). Roles COORDINADOR, PROGRAMADOR.
 *     tags: [Complementarias]
 *     security:
 *       - token: []
 *     responses:
 ...
 */
routerComplementary.get("/email-templates", validateGetEmailTemplates, getEmailTemplates);

/**
 * @swagger
 * /api/complementary/email-templates/{key}:
 *   get:
 *     summary: Obtiene el detalle completo de un template por su key
 *     tags: [Complementarias]
 *     security:
 *       - token: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Template completo con htmlContent
 *       404:
 *         description: Template no encontrado
 */
routerComplementary.get("/email-templates/:key", validateGetEmailTemplateByKey, getEmailTemplateByKey);

/**
 * @swagger
 * /api/complementary/email-templates/{key}:
 *   put:
 *     summary: Actualiza el subject y/o htmlContent de un template de correo
 *     tags: [Complementarias]
 *     security:
 *       - token: []
 *     parameters:
 ...
 *         description: Template no encontrado
 */
routerComplementary.put("/email-templates/:key", validateUpdateEmailTemplate, updateEmailTemplate);

/**
 * @swagger
 * /api/complementary/email-templates/{key}/reset:
 *   post:
 *     summary: Restaura un template de correo a sus valores originales
 *     tags: [Complementarias]
 *     security:
 *       - token: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Template restaurado
 *       404:
 *         description: Template no encontrado
 */
routerComplementary.post("/email-templates/:key/reset", validateResetEmailTemplate, resetEmailTemplate);

// ==================== Preferencias de Notificación (Coordinador/Programador) ====================

/**
 * @swagger
 * /api/complementary/notification-prefs:
 *   get:
 *     summary: Obtiene las preferencias de notificación del usuario autenticado
 *     description: Solo accesible por COORDINADOR y PROGRAMADOR.
 *     tags: [Complementarias]
 *     security:
 *       - token: []
 *     responses:
 ...
 */
routerComplementary.get("/notification-prefs", validateGetNotificationPrefs, getMyNotificationPrefs);

/**
 * @swagger
 * /api/complementary/notification-prefs:
 *   put:
 *     summary: Actualiza las preferencias de notificación del usuario autenticado
 *     description: Permite habilitar/deshabilitar correos automáticos y configurar un email destino alternativo.
 *     tags: [Complementarias]
 *     security:
 *       - token: []
 *     requestBody:
 ...
 *               habilitado:
 *                 type: boolean
 *                 description: "true = recibe correos automáticos; false = no recibe"
 *               emailDestino:
 *                 type: string
 *                 nullable: true
 *                 description: "Email alternativo destino. null para usar el email de la cuenta."
 *     responses:
 *       200:
 *         description: Preferencias actualizadas
 */
routerComplementary.put("/notification-prefs", validateUpdateNotificationPrefs, updateMyNotificationPrefs);
