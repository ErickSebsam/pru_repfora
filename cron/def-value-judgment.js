...
// Tipos de novedad que excluyen al aprendiz de los listados de pendientes
const EXCLUDED_TPNEW = ['DESERCIÓN', 'RETIRO VOLUNTARIO', 'TRASLADO', 'APLAZAMIENTO'];

/**
 * Devuelve la info de novedades excluyentes para una ficha.
 * Resultado: Map<documentoNormalizado, tpnew>
 * No filtra por state para excluir en cualquier estado (REGISTRADA, APROBADA, NO APROBADA).
 */
async function getExcludedNewsInfo(ficheId) {
  if (!ficheId) return new Map();
  const rows = await News.find({
    fiche: ficheId,
    tpnew: { $in: EXCLUDED_TPNEW }
  }).select('document tpnew').lean();
  const map = new Map();
  for (const n of rows) {
    const doc = String(n.document || '').toUpperCase().trim();
    if (doc) map.set(doc, n.tpnew);
  }
  return map;
}

/**
 * Normaliza un documento para comparación (mismo criterio que getExcludedNewsInfo y buildFicheSummary).
 */
function normalizeDoc(doc) {
  return String(doc || '').toUpperCase().trim();
}
...
async function processWithLocalReport(localReportPath, groupsToProcess, options) {
  ...
  for (const group of groupsToProcess) {
    ...
    // Cargar novedades excluyentes una vez por ficha
    if (!group.newsExcludedInfo) {
      group.newsExcludedInfo = await getExcludedNewsInfo(group.ficheId);
    }
    const newsInfo = group.newsExcludedInfo;
    ...
    for (const sched of schedules) {
      ...
      if (!analysis.foundColumn) {
        ...
        // Filtrar aprendices omitidos (LearnerOmission) primero — preserva comportamiento previo
        const omittedDocs = sched.omittedLearners || [];
        const afterOmission = analysis.missingLearners.filter(l =>
          !omittedDocs.includes(l.document)
        );

        const afterOmissionCount = afterOmission.length;
        ...
        if (afterOmissionCount === 0 && analysis.missingLearners.length > 0) {
          ...
          // Separar aprendices con novedad (News) de los que se reportan
          const newsExcludedLearners = [];
          const filteredLearners = [];

          if (newsInfo && newsInfo.size > 0) {
            for (const l of afterOmission) {
              const norm = normalizeDoc(l.document);
              if (newsInfo.has(norm)) {
                newsExcludedLearners.push({ ...l, noveltyType: newsInfo.get(norm) });
              } else {
                filteredLearners.push(l);
              }
            }
          } else {
            filteredLearners.push(...afterOmission);
          }

          // Verificar si está vencido (se calcula siempre, también para excluidos)
          const now = new Date();
          ...
          // Registrar excluidos por novedad en el listado aparte
          if (newsExcludedLearners.length > 0 && debugLog.newsExcluded) {
            for (const l of newsExcludedLearners) {
              debugLog.newsExcluded.push({
                scheduleId: sched.scheduleId,
                ficheNumber,
                ficheId: group.ficheId,
                outcomeText: sched.outcomeText,
                fend: sched.fend,
                learnerName: l.name,
                learnerDocument: l.document,
                noveltyType: l.noveltyType,
                daysOverdue,
                isVencido,
            ...
            log('ANALYZE', `[NOVEDAD] ${newsExcludedLearners.length} aprendiz(ces) excluido(s) por novedad en este RAP`);
          }

          // Si tras excluir novedades no queda nadie para reportar, NO marcamos rated
          if (filteredLearners.length === 0) {
            outcomeDebug.result = 'SKIPPED_BY_NEWS';
            log('ANALYZE', `[SKIP] RAP sin aprendices a reportar tras excluir novedades`);
          } else {
...
async function reviewJudgmentCore(config) {
  ...
  await postProcessing.updateState(
    accumulators.pendientes,
    accumulators.vencidos,
    accumulators.fichesSummary || [],
    accumulators.newsExcluded || []
  );
...
async function buildFicheSummary({
  ...
  // No se filtra por state: se excluye en cualquier estado (REGISTRADA, APROBADA, NO APROBADA)
  const newsInfo = group.newsExcludedInfo || await getExcludedNewsInfo(ficheId);
  const excludedDocs = new Set(newsInfo.keys());
...
async function processGroup(group, client, config) {
  ...
  try {
    ...
    // Cargar novedades excluyentes una vez por ficha (reutilizado en processSchedule y buildFicheSummary)
    if (!group.newsExcludedInfo) {
      group.newsExcludedInfo = await getExcludedNewsInfo(group.ficheId);
    }
...
async function processSchedule(sched, indexedResults, group, config) {
  ...
  // === DETECTADO PENDIENTE ===
  const omittedDocs = sched.omittedLearners || [];
  // Filtrar primero por omisiones manuales (LearnerOmission) — preserva comportamiento previo
  const afterOmission = analysis.missingLearners.filter(l =>
    !omittedDocs.includes(l.document)
  );

  const missingCount = afterOmission.length;
  const totalCount = analysis.totalEnFormacion;

  // Si después de filtrar por omisiones no queda nadie, está calificado (comportamiento previo)
  ...
  // Separar aprendices con novedad excluyente (News) — no aparecen en listados,
  // pero se registran aparte en accumulators.newsExcluded
  const newsInfo = group.newsExcludedInfo;
  const newsExcludedLearners = [];
  const filteredLearners = [];
  ...
  // Registrar aprendices excluidos por novedad en el listado aparte
  if (newsExcludedLearners.length > 0 && accumulators.newsExcluded) {
    for (const l of newsExcludedLearners) {
      accumulators.newsExcluded.push({
        scheduleId: sched.scheduleId,
        ficheNumber,
        ficheId: group.ficheId,
        outcomeText: sched.outcomeText,
    ...
    logger.info('ANALYZE', `[NOVEDAD] ${newsExcludedLearners.length} aprendiz(ces) excluido(s) por novedad en este RAP`);
  }

  // Si tras excluir novedades no queda nadie para reportar, NO marcamos rated (pueden volver)
  if (filteredLearners.length === 0) {
    logger.info('ANALYZE', `[SKIP] RAP sin aprendices a reportar tras excluir novedades — no se agrega a pendientes/vencidos`);
    return;
  }

  // Recalcular totales sobre los que efectivamente se reportarán
  const reportMissingCount = filteredLearners.length;
  const reportIsTotalMissing = totalCount > 0 && reportMissingCount >= totalCount;

  // Para el correo y log: si faltan todos, usar mensaje genérico
  const learnersForReport = reportIsTotalMissing
    ? [{ name: '(Todos)', document: 'Resultado sin evaluar' }]
    : filteredLearners;
  ...
  const outcomeState = buildOutcomeState({
    ...
    isTotalMissing: reportIsTotalMissing,
    daysOverdue,
    instructorName: schedInstructor?.name || 'Sin instructor',
    instructorEmail: schedInstructor?.email || '',
    ficheOwnerName,
  ...
  if (reportIsTotalMissing) {
    logger.info('ANALYZE', `[PENDIENTE] Resultado sin evaluar (${totalCount} aprendices)`);
  } else {
    logger.info('ANALYZE', `[PENDIENTE] Faltan ${reportMissingCount} de ${totalCount} aprendices`);
  }
  ...
  if (accumulators.debugLog?.summary) {
    ...
    accumulators.debugLog.summary.totalMissingLearners += reportMissingCount;
  }
  ...
  if (isVencido && accumulators.debugLog?.vencedOutcomes) {
    const vencedOutcome = {
      ...
      fend: sched.fend,
      daysOverdue,
      missingCount: reportMissingCount,
      instructorName: schedInstructor?.name || '',
      instructorEmail: schedInstructor?.email || '',
      ficheOwnerName,
      ficheOwnerEmail,
...
export async function reviewJudgmentDebug(options = {}) {
  ...
  const config = {
    ...
    accumulators: {
      ...
      newsExcluded: debugLog.newsExcluded,
      debugLog
    },
    postProcessing: {
      ...
      updateState: async (pend, venc, summary, newsExcludedArg) => {
        try {
          await updateCurrentAuditState(pend, venc, summary, newsExcludedArg || []);
          logger.info('UPDATE_STATE', `Registro único actualizado: ${pend.length} pendientes, ${venc.length} vencidos, ${summary?.length || 0} fichas resumidas, ${(newsExcludedArg || []).length} excluidos por novedad`);
        } catch (error) {
  ...
  debugLog.summary.totalExcludedByNews = debugLog.newsExcluded.length;
  return debugLog;
}
  ...
  const config = {
    ...
    postProcessing: {
      ...
      updateState: async (pend, venc) => {
        try {
          await updateCurrentAuditState(pend, venc, fichesSummary, newsExcluded);
          console.log(`[AUDIT_STATE] Ejecución guardada: ${pend.length} pendientes, ${venc.length} vencidos, ${fichesSummary.length} fichas resumidas, ${newsExcluded.length} excluidos por novedad`);
        } catch (error) {
      ...
      sendCoordinatorReports: async (map) => {
        ...
        for (const [coordId, report] of map.entries()) {
          ...
          const coordNewsExcluded = newsExcluded.filter(n => coordFicheNumbers.has(String(n.ficheNumber)));
          const result = await sendCoordinatorReport(report.coordination, report.fichas, coordFichesSummary, coordPendientes, coordVencidos, coordNewsExcluded);
          if (result.success) {
            coordSent++;
          } else {
            coordFailed++;
