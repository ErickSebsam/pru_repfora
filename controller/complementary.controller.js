...
import { getDf14aCronStatus, startDf14aCron, stopDf14aCron } from "../services/cronService.js";
...
import { notifyApproval, notifyRejection, notifyFichaAssigned, notifyNewRequest, notifyCancellation, notifyScheduled, notifyResubmit, notifyExecution, notifyExtensionRequest, notifyExtensionResolved, notifyMatriculada } from "../services/complementaryNotificationService.js";
...
compCtrl.getCatalogs = async (req, res) => {
  ...
  try {
    ...
    // Decodificamos el token para saber quién realiza la petición
    const token = req.headers?.token || req.headers?.authorization;
    let isInstructor = false;
    if (token) {
      try {
        const decoded = jwt.decode(token.replace("Bearer ", ""));
        // El scope "VERIFY" lo usan los instructores para complementarias
        if (decoded?.scope === "VERIFY" || decoded?.rol === "INSTRUCTOR") {
          isInstructor = true;
        }
      } catch (e) {}
    }

    // Validación: los instructores SOLO ven programas habilitados (status: 0),
    // y para la ruta general (sin filtro status explícito) también solo se envían los habilitados.
    if (isInstructor) {
      filter.status = 0;
    } else {
      if (status !== undefined && status !== "") {
        ...
        filter.status = 0; // Descartar inhabilitados por defecto
      }
    }
    ...
    if (esModoPaginado) {
      ...
      if (qTrim) {
        filter.$or = [
          { prfCodigoStr: { $regex: qTrim, $options: "i" } },
          { prfDenominacion: { $regex: qTrim, $options: "i" } }
        ];
      }
    } else {
...
compCtrl.registerRequest = async (req, res) => {
  ...
  try {
    ...
    let maxDuracion;
    let finalCourseCode, finalCourseName, finalCourseVersion;

    if (req.body.prfCodigo && req.body.prfCodigo !== "") {
      finalCourseCode = String(req.body.prfCodigo);
      finalCourseName = req.body.prfDenominacion;
      finalCourseVersion = String(req.body.prfVersion);
      maxDuracion = Number(req.body.prfDuracionMaxima) || 0;
    } else {
      finalCourseCode = String(catalog.prfCodigo);
      finalCourseName = catalog.prfDenominacion;
      finalCourseVersion = String(catalog.prfVersion);
      maxDuracion = Number(catalog.prfDuracionMaxima) || Number(prfDuracionMaxima) || 0;
    }

    complementaryHelper.validateSesionesHours(mappedSesiones, maxDuracion);

    let finalEnvironment = null;
    let finalAmbienteNombre = (req.body.ambienteNombre || "").toUpperCase().trim();
    if (environment && /^[0-9a-fA-F]{24}$/.test(environment)) {
      const envObj = await Environment.findById(environment);
      if (envObj) {
        finalEnvironment = envObj._id;
        finalAmbienteNombre = envObj.name;
      } else {
        finalEnvironment = environment;
      }
    } else if (req.body.ambienteNombre && req.body.ambienteNombre.trim() !== "") {
      finalAmbienteNombre = req.body.ambienteNombre.trim().toUpperCase();
      finalEnvironment = null;
    }

    const newRequest = new ComplementaryRequest({
      catalogCourse,
      catalogCourseName: finalCourseName,
      catalogCourseCode: finalCourseCode,
      catalogCourseVersion: finalCourseVersion,
      ...
      supervisorNombre,
      ambienteNombre: finalAmbienteNombre,
    ...
    // Generar numeroSolicitud consecutivo si no viene del frontend
    if (req.body.numeroSolicitud) {
      newRequest.numeroSolicitud = req.body.numeroSolicitud;
    } else {
    ...
    const { solicitudPdf, cedulas, cedulasPdf, formatoCreacion, carta, adicional } = req.files;
    const attachments = [
      {
        filename: `Solicitud_${newRequest.numeroSolicitud}.pdf`,
        path: solicitudPdf.tempFilePath,
        contentType: "application/pdf",
      },
      {
        filename: `Cedulas_${newRequest.numeroSolicitud}${path.extname(cedulas.name)}`,
        path: cedulas.tempFilePath,
        contentType: cedulas.mimetype,
      },
      {
        filename: `CedulasPDF_${newRequest.numeroSolicitud}.pdf`,
        path: cedulasPdf.tempFilePath,
        contentType: "application/pdf",
      },
    ];
    ...
    if (formatoCreacion) {
      attachments.push({
        filename: `FormatoCreacion_${newRequest.numeroSolicitud}${path.extname(formatoCreacion.name || "")}`,
        path: formatoCreacion.tempFilePath,
        contentType: formatoCreacion.mimetype,
      });
    }
    if (carta) {
      attachments.push({
        filename: `Carta_${newRequest.numeroSolicitud}${path.extname(carta.name || "")}`,
        path: carta.tempFilePath,
        contentType: carta.mimetype,
      });
    }
    if (adicional) {
      attachments.push({
        filename: `Adicional_${newRequest.numeroSolicitud}${path.extname(adicional.name || "")}`,
        path: adicional.tempFilePath,
        contentType: adicional.mimetype,
      });
    }

    notifyNewRequest(newRequest, instructorName, attachments).catch((err) => {
...
compCtrl.getRequests = async (req, res) => {
  ...
  try {
    ...
    let query = ComplementaryRequest.find(filter)
      .select("numeroSolicitud codigoSolicitud fichaNumber fichaCaracterizacion fechaInicio fechaFin fechaInscripcionInicio fechaInscripcionFin fechaMatriculaInicio fechaMatriculaFin catalogCourseName catalogCourseCode state municipio ambienteNombre instructor instructores environment visto puedeReprogramar createdAt")
...
compCtrl.getRequestCounts = async (req, res) => {
  try {
    ...
    if (isInstructor || decoded.rol === "INSTRUCTOR") {
      const instructorDoc = await complementaryHelper.findInstructorByEmail(decoded.id);
      if (!instructorDoc) {
        return res.status(401).json({ msg: "Instructor no encontrado" });
      }
      match.$or = [
...
compCtrl.getRequestId = async (req, res) => {
  ...
  try {
    ...
    const request = await ComplementaryRequest.findById(id)
      ...
      .populate("history.changedBy", "name rol")
      .populate("environment", "name");
    ...
    // Resolver changedBy nulos en el historial: ocurre cuando el actor es un Instructor
    // (ref a colección Instructor, no User), por lo que el populate anterior devuelve null.
    // Se consulta el documento raw para obtener los ObjectIds originales de changedBy.
    const requestObj = request.toObject();
    const hasNullChangedBy = requestObj.history.some(h => h.changedBy === null);

    if (hasNullChangedBy) {
      const raw = await ComplementaryRequest.findById(id, { history: 1 }).lean();
      const rawHistoryMap = Object.fromEntries(
        (raw?.history || []).map(h => [h._id.toString(), h.changedBy])
      );
      const missingIds = [];
      for (const h of requestObj.history) {
        if (h.changedBy === null) {
          const rawId = rawHistoryMap[h._id.toString()];
          if (rawId) missingIds.push(rawId);
        }
      }
      if (missingIds.length > 0) {
        const instructores = await Instructor.find({ _id: { $in: missingIds } }, "name");
        const instructorMap = Object.fromEntries(instructores.map(inst => [inst._id.toString(), inst]));
        for (const h of requestObj.history) {
          if (h.changedBy === null) {
            const rawId = rawHistoryMap[h._id.toString()];
            if (rawId && instructorMap[rawId.toString()]) {
              h.changedBy = {
                _id: rawId,
                name: instructorMap[rawId.toString()].name,
              };
            }
          }
        }
...
compCtrl.getInstructorRequests = async (req, res) => {
  try {
    ...
    const requests = await ComplementaryRequest.find({
      ...
      .select("numeroSolicitud codigoSolicitud fichaNumber fichaCaracterizacion fechaInicio fechaFin catalogCourseName catalogCourseCode state municipio ambienteNombre instructor instructores environment visto puedeReprogramar createdAt")
      .populate("catalogCourse", "prfDenominacion prfCodigo prfVersion")
      .populate("instructor", "name email numdocument")
      .populate("instructores.instructor", "name email numdocument")
      .populate("environment", "name")
...
compCtrl.updateRequest = async (req, res) => {
  ...
  try {
    const decoded = await webToken.decodeAnyToken(req.headers.token);
    if (!decoded.isInstructor) {
      return await compCtrl.adminUpdateRequest(req, res);
    }

    let mappedSesiones = undefined;
    if (sesiones) {
      mappedSesiones = (sesiones || []).map((s) => {
        const rawRes = s.resultados || s.resultado || [];
        const resultadosArray = Array.isArray(rawRes) ? rawRes : [rawRes];
        return {
          fecha: s.fecha || "",
          ...
          horaFin: s.horaFin || "",
          totalHoras: s.totalHoras || 0,
          instructor: s.instructor || decoded.id,
          competencia: (s.competencia || "").toUpperCase().trim(),
          resultados: resultadosArray.map((r) => String(r).toUpperCase().trim()),
          actividadAprendizaje: (s.actividadAprendizaje || "").toUpperCase().trim(),
        };
    ...
    let finalEnvironment = undefined;
    let finalAmbienteNombre = req.body.ambienteNombre !== undefined ? (req.body.ambienteNombre || "").toUpperCase().trim() : undefined;
    if (environment !== undefined || req.body.ambienteNombre !== undefined) {
      if (environment && /^[0-9a-fA-F]{24}$/.test(environment)) {
        const envObj = await Environment.findById(environment);
        if (envObj) {
          finalEnvironment = envObj._id;
    ...
    await ComplementaryRequest.findByIdAndUpdate(id, {
      ...complementaryHelper.normalizeRequestFields(req.body),
      ...(finalAmbienteNombre !== undefined ? { ambienteNombre: finalAmbienteNombre } : {}),
      ...(finalEnvironment !== undefined ? { environment: finalEnvironment } : {}),
      formationDocument: formationDocument || "",
      competencies: complementaryHelper.normalizeCompetencies(competencies || []),
      outcomes: outcomes || [],
      fechaInicio: fechaInicio ? new Date(fechaInicio) : null,
      fechaFin: fechaFin ? new Date(fechaFin) : null,
      fechaInscripcionInicio: fechaInscripcionInicio || null,
      fechaInscripcionFin: fechaInscripcionFin || null,
      fechaMatriculaInicio: fechaMatriculaInicio || null,
      fechaMatriculaFin: fechaMatriculaFin || null,
      numAprendices: numAprendices || 0,
      ...(mappedSesiones !== undefined ? { sesiones: mappedSesiones } : {}),
    });
    ...
    const updatedRequest = await ComplementaryRequest.findById(id).populate([
      { path: "catalogCourse" },
      { path: "instructor", select: "-password" },
      { path: "instructores.instructor", select: "-password" },
    ...
    if (updatedRequest?.fichaNumber) {
      await complementaryHelper.syncFicheFromRequest(updatedRequest).catch((err) => {
        console.error("Error sincronizando Fiche en updateRequest:", err);
      });
    }

    res.json({ msg: "Solicitud actualizada correctamente", data: updatedRequest });
  } catch (error) {
    console.log(error);
    res.status(400).json({ msg: "No fue posible terminar la operacion" });
  }
...
compCtrl.resubmitRequest = async (req, res) => {
  ...
  try {
    ...
    const instructor = await complementaryHelper.findInstructorByEmail(decoded.id);
    const instructorName = instructor?.name || decoded.email;
    notifyResubmit(request, instructorName).catch((err) => {
      console.error("Error enviando correo de reenvio de solicitud:", err);
    });
...
compCtrl.approveRequest = async (req, res) => {
  ...
  try {
    ...
    notifyApproval(request).catch((err) => {
      console.error("Error enviando correo de aprobacion de solicitud:", err);
    });
...
compCtrl.rejectRequest = async (req, res) => {
  ...
  try {
    ...
    notifyRejection(request, observations).catch((err) => {
      console.error("Error enviando correo de rechazo de solicitud:", err);
    });
...
compCtrl.assignFicha = async (req, res) => {
  ...
  try {
    ...
    const request = await ComplementaryRequest.findById(id);
    if (!request || ["CERRADA", "CANCELADA"].includes(request.state)) {
      return res
        .status(400)
        .json({ msg: "La solicitud no existe o se encuentra en estado CERRADA o CANCELADA" });
    }

    const fichaInput = (fichaCaracterizacion !== undefined && fichaCaracterizacion !== null && String(fichaCaracterizacion).trim() !== "")
      ? String(fichaCaracterizacion).trim()
      : (fichaNumber !== undefined && fichaNumber !== null && String(fichaNumber).trim() !== "")
      ? String(fichaNumber).trim()
      : "";

    if (fichaInput !== "") {
      const normalizedNumber = fichaInput.toUpperCase();
      request.fichaCaracterizacion = normalizedNumber;
      request.fichaNumber = normalizedNumber;
      if (fechaInicio !== undefined) request.fechaInicio = fechaInicio ? new Date(fechaInicio) : null;
      if (fechaFin !== undefined) request.fechaFin = fechaFin ? new Date(fechaFin) : null;

      newFiche = await complementaryHelper.syncFicheFromRequest(request, {
        fichaNumber: normalizedNumber,
        fechaInicio: request.fechaInicio,
        fechaFin: request.fechaFin,
      });
    }

    if (codigoSolicitud !== undefined && codigoSolicitud !== null && String(codigoSolicitud).trim() !== "") {
      request.codigoSolicitud = String(codigoSolicitud).toUpperCase().trim();
    }
    if (proyectoAsociado !== undefined && proyectoAsociado !== null) {
      request.proyectoAsociado = String(proyectoAsociado).toUpperCase().trim();
    }
    if (actaTrazabilidad !== undefined && actaTrazabilidad !== null) {
      request.actaTrazabilidad = String(actaTrazabilidad).toUpperCase().trim();
    }
    if (tipoPrograma !== undefined) request.tipoPrograma = tipoPrograma ? tipoPrograma.trim() : "";
    if (tipoPoblacion !== undefined) request.tipoPoblacion = tipoPoblacion ? tipoPoblacion.trim() : "";
    if (fechaInicio !== undefined) request.fechaInicio = fechaInicio ? new Date(fechaInicio) : null;
    if (fechaFin !== undefined) request.fechaFin = fechaFin ? new Date(fechaFin) : null;
    if (fechaInscripcionInicio !== undefined) request.fechaInscripcionInicio = fechaInscripcionInicio ? new Date(fechaInscripcionInicio) : null;
    if (fechaInscripcionFin !== undefined) request.fechaInscripcionFin = fechaInscripcionFin ? new Date(fechaInscripcionFin) : null;
    if (fechaMatriculaInicio !== undefined) request.fechaMatriculaInicio = fechaMatriculaInicio ? new Date(fechaMatriculaInicio) : null;
    if (fechaMatriculaFin !== undefined) request.fechaMatriculaFin = fechaMatriculaFin ? new Date(fechaMatriculaFin) : null;

    if (environment !== undefined || ambienteNombre !== undefined) {
      if (environment && /^[0-9a-fA-F]{24}$/.test(environment)) {
        const envObj = await Environment.findById(environment);
        request.environment = envObj ? envObj._id : environment;
        request.ambienteNombre = envObj ? envObj.name : "";
      } else if (ambienteNombre && ambienteNombre.trim() !== "") {
        request.ambienteNombre = ambienteNombre.trim().toUpperCase();
        request.environment = null;
      } else {
        request.environment = null;
        request.ambienteNombre = "";
      }
    }
    ...
    if (schedule) {
      if (newFiche) {
        schedule.fiche = newFiche._id;
      }
      if (environment !== undefined || ambienteNombre !== undefined) {
        schedule.environment = request.environment || undefined;
      }
      await schedule.save();
    }

    if (["APROBADA", "PROGRAMADA", "PENDIENTE", "EN_REVISION"].includes(request.state)) {
      const previousState = request.state;
      request.state = "CODIGO_ASIGNADO";
      request.history.push({
        previousState: previousState,
        newState: "CODIGO_ASIGNADO",
        changedBy: decoded.id,
        changedByRole: decoded.rol,
        observations: `Código asignado — ${request.numeroSolicitud}`,
      });
    } else {
      request.history.push({
        ...
        changedBy: decoded.id,
        changedByRole: decoded.rol,
        observations: `Datos de ficha/fechas actualizados — ${request.numeroSolicitud}`,
      });
    }
    await request.save();
    ...
    notifyFichaAssigned(request).catch((err) => {
      console.error("Error enviando correo de asignacion de ficha:", err);
    });
    ...
    if (
      ...
      (error.message.includes("ya existe") ||
        error.message.includes("ya está asociado"))
    ) {
...
compCtrl.addFormationData = async (req, res) => {
  ...
  try {
    ...
    if (request.state !== "APROBADA" && request.state !== "CODIGO_ASIGNADO" && request.state !== "FICHA_ASIGNADA") {
      return res.status(400).json({
        msg: "La solicitud debe estar en estado APROBADA o CODIGO_ASIGNADO para agregar datos de formación",
      });
    }
...
compCtrl.changeState = async (req, res) => {
  ...
  try {
    ...
    if (newState === "CANCELADA") {
      notifyCancellation(request, previousState, observations || "").catch((err) => {
        console.error("Error enviando correo de cancelacion de solicitud:", err);
      });
    }

    // Notificación cuando la ficha pasa a EJECUCION
    if (newState === "EJECUCION") {
      notifyExecution(request).catch((err) => {
        console.error("Error enviando correo de ejecucion de solicitud:", err);
      });
    }

    // Notificación cuando la ficha pasa a MATRICULADA
    if (newState === "MATRICULADA") {
      notifyMatriculada(request).catch((err) => {
        console.error("Error enviando correo de matriculacion de solicitud:", err);
      });
    }
...
compCtrl.closeFicha = async (req, res) => {
  ...
  try {
    ...
    request.history.push({
      ...
      changedBy: decoded.id,
      changedByRole: decoded.rol,
      observations: "Ficha complementaria cerrada",
    });
    await request.save();

    if (request.fichaNumber) {
      await complementaryHelper.syncFicheFromRequest(request, {
        status: 1,
      }).catch((err) => {
        console.error("Error sincronizando Fiche en closeFicha:", err);
      });
    }
...
compCtrl.scheduleComplementary = async (req, res) => {
  ...
  try {
    ...
    await complementaryScheduleHelper.validateInstructorAvailability(
      instructor, fstart, fend, tstart, tend, days, null, sesiones
    );
    ...
    if (environment) {
      await complementaryScheduleHelper.validateEnvironmentAvailability(
        environment, fstart, fend, tstart, tend, days, null, sesiones
      );
    }

    // 5. Calcular horas de trabajo y fechas de eventos
    let eventDates = [];
    if (sesiones && sesiones.length > 0) {
      eventDates = sesiones
        .filter((s) => !s.instructor || String(s.instructor) === String(instructor))
        .map((s) => s.fecha)
        .filter(Boolean);
    ...
    if (sesiones && Array.isArray(sesiones) && sesiones.length > 0) {
      const instSessions = sesiones.filter((s) => !s.instructor || String(s.instructor) === String(instructor));
      if (instSessions.length > 0) {
        hourswork = instSessions.reduce((sum, s) => {
          if (s.totalHoras !== undefined && s.totalHoras !== null && !isNaN(Number(s.totalHoras))) {
            return sum + Number(s.totalHoras);
          }
          if (s.horaInicio && s.horaFin) {
            return sum + Number(calculateNumHoursWork(s.horaInicio, s.horaFin, 1));
          }
          return sum + Number(calculateNumHoursWork(tstart, tend, 1));
        }, 0);
      } else {
        hourswork = calculateNumHoursWork(tstart, tend, eventDates.length);
      }
    } else {
      hourswork = calculateNumHoursWork(tstart, tend, eventDates.length);
    }
    ...
    const newSchedule = new Schedule({
      ...
      environment: environment && /^[0-9a-fA-F]{24}$/.test(environment) ? environment : undefined,
      days,
    ...
    // 10. Actualizar fechaInicio, fechaFin y sesiones de la solicitud (sincronización)
    const effectiveFstart = eventDates.length > 0 ? new Date(eventDates[0]) : new Date(fstart);
    const effectiveFend = eventDates.length > 0 ? new Date(eventDates[eventDates.length - 1]) : new Date(fend);

    request.fechaInicio = effectiveFstart;
    request.fechaFin = effectiveFend;

    const reqOutcomes = (request.outcomes || []).filter(Boolean);

    if (sesiones && sesiones.length > 0) {
      const totalSes = sesiones.length;
      request.sesiones = sesiones.map((s, index) => {
        const rawRes = s.resultados || s.resultado || [];
        const resultadosArray = (Array.isArray(rawRes) ? rawRes : [rawRes]).filter(Boolean);
        let finalRes = resultadosArray.map((r) => String(r).toUpperCase().trim());

        if (finalRes.length === 0 && reqOutcomes.length > 0) {
          const outcomeIdx = Math.min(
            Math.floor((index * reqOutcomes.length) / totalSes),
            reqOutcomes.length - 1
          );
          finalRes = [String(reqOutcomes[outcomeIdx]).toUpperCase().trim()];
        }

        return {
          fecha: s.fecha || "",
          horaInicio: s.horaInicio || tstart,
          horaFin: s.horaFin || tend,
          totalHoras: s.totalHoras || calculateNumHoursWork(s.horaInicio || tstart, s.horaFin || tend, 1),
          instructor: s.instructor || instructor,
          competencia: sanitizeCompetence((s.competencia || "").toUpperCase().trim()),
          resultados: finalRes.map(r => sanitizeOutcome(r)),
          actividadAprendizaje: (s.actividadAprendizaje || "").toUpperCase().trim(),
        };
      });
      ...
      const totalSes = eventDates.length;
      request.sesiones = eventDates.map((date, index) => {
        const existing = previousSesiones[index];
        let existingRes = (existing?.resultados || []).filter(Boolean);
        if (existingRes.length === 0 && reqOutcomes.length > 0) {
          const outcomeIdx = Math.min(
            Math.floor((index * reqOutcomes.length) / totalSes),
            reqOutcomes.length - 1
          );
          existingRes = [String(reqOutcomes[outcomeIdx]).toUpperCase().trim()];
        }
        return {
          ...
          resultados: existingRes,
          actividadAprendizaje: existing?.actividadAprendizaje || "",
        };
      });
    }

    // 11. Avanzar estado si es necesario (solo desde APROBADA)
    if (request.state === "APROBADA") {
      const previousState = request.state;
      ...
      request.history.push({
        ...
        changedBy: decoded.id,
        changedByRole: decoded.rol,
        observations: `Horario inicial programado. Rango: ${effectiveFstart.toLocaleDateString("es-CO")} - ${effectiveFend.toLocaleDateString("es-CO")}`,
      });
    }
    await request.save();

    // 12. Notificar al instructor por correo
    notifyScheduled(request, newSchedule).catch((err) => {
      console.error("Error enviando correo de horario programado:", err);
    });
...
compCtrl.getInstructorAllSchedules = async (req, res) => {
  ...
  try {
    ...
    const complementariaRaw = await Schedule.find({
      instructor: instructorId,
      scheduleType: "COMPLEMENTARIA",
      status: 0,
    })
      .populate("instructor", "name email numdocument")
      .populate("environment", "name")
      .populate("complementaryRequest", "fichaNumber catalogCourseName state outcomes sesiones competencies")
      .sort({ fstart: 1 });

    const complementaria = complementariaRaw.filter((s) => {
      if (s.complementaryRequest) {
        return !["CANCELADA", "RECHAZADA", "CERRADA"].includes(s.complementaryRequest.state);
      }
      return true;
    });
...
compCtrl.getFichasSinRuta = async (req, res) => {
  ...
  try {
    const filter = {
      state: { $in: ["CODIGO_ASIGNADO", "FICHA_ASIGNADA", "INSCRIPCION"] },
      status: 0,
    };
...
compCtrl.getDashboardSummary = async (req, res) => {
  try {
    // 1. Fichas sin ruta (solicitudes en estado CODIGO_ASIGNADO o INSCRIPCION sin Schedule activo)
...
compCtrl.rescheduleFicha = async (req, res) => {
  const { id } = req.params;
  const { fstart, fend, tstart, tend, days, events, sesiones, environment, supporttext } = req.body;
  try {
    const decoded = await webToken.decodeAnyToken(req.headers.token);
    ...
    // 4. Filtrar eventos (soportando tanto sesiones del payload nuevo como events del fallback)
    let eventDates = [];
    if (sesiones && sesiones.length > 0) {
      eventDates = sesiones
        .filter((s) => String(s.instructor) === String(schedule.instructor))
        .map((s) => s.fecha)
        .filter(Boolean);
    } else if (events && events.length > 0) {
      eventDates = events
    ...
    await complementaryScheduleHelper.validateInstructorAvailability(
      schedule.instructor, fstart, fend, tstart, tend, days, id, sesiones
    );

    // 6. Disponibilidad del ambiente (si viene)
    if (environment) {
      await complementaryScheduleHelper.validateEnvironmentAvailability(
        environment, fstart, fend, tstart, tend, days, id, sesiones
      );
    }

    // 7. Horas nuevas calculadas exactamente con suma por bloques de sesiones o fallback
    let newHours = 0;
    if (sesiones && Array.isArray(sesiones) && sesiones.length > 0) {
      const instSessions = sesiones.filter((s) => !s.instructor || String(s.instructor) === String(schedule.instructor));
      if (instSessions.length > 0) {
        newHours = instSessions.reduce((sum, s) => {
          if (s.totalHoras !== undefined && s.totalHoras !== null && !isNaN(Number(s.totalHoras))) {
            return sum + Number(s.totalHoras);
          }
          if (s.horaInicio && s.horaFin) {
        ...
        newHours = calculateNumHoursWork(tstart, tend, eventDates.length);
      }
    } else {
      newHours = calculateNumHoursWork(tstart, tend, eventDates.length);
    }
    ...
    if (environment !== undefined) {
      schedule.environment = environment || undefined;
    }
    if (supporttext) schedule.supporttext = supporttext;
    await schedule.save();
    ...
    if (sesiones && sesiones.length > 0) {
      request.sesiones = sesiones.map((s) => {
        ...
        return {
          ...
          horaFin: s.horaFin || tend,
          totalHoras: s.totalHoras || calculateNumHoursWork(s.horaInicio || tstart, s.horaFin || tend, 1),
          instructor: s.instructor || schedule.instructor,
          competencia: (s.competencia || "").toUpperCase().trim(),
          resultados: resultadosArray.map((r) => String(r).toUpperCase().trim()),
          actividadAprendizaje: (s.actividadAprendizaje || "").toUpperCase().trim(),
        };
    ...
    if (request.puedeReprogramar) {
      request.puedeReprogramar = false;
    }
    await request.save();

    if (request.fichaNumber) {
      await complementaryHelper.syncFicheFromRequest(request, {
        fechaInicio: effectiveFstart,
        fechaFin: effectiveFend,
      }).catch((err) => {
        console.error("Error sincronizando Fiche en rescheduleFicha:", err);
      });
    }
...
compCtrl.getComplementariasPorFecha = async (req, res) => {
  ...
  try {
    ...
    // Buscar solicitudes asociadas al nombre del municipio (para soportar schedules sin ambiente)
    let requestsInTownIds = [];
    const townDoc = await Town.findById(town);
    if (townDoc) {
      const townNameNormalized = townDoc.name.toUpperCase().trim();
      const requestsInTown = await ComplementaryRequest.find({
        municipio: townNameNormalized,
        status: 0,
      }).select("_id");
      requestsInTownIds = requestsInTown.map((r) => r._id);
    }

    // Construir el query híbrido
    ...
    if (ambienteIds.length > 0) {
      schedulesQuery.$or.push({ environment: { $in: ambienteIds } });
    }
    if (requestsInTownIds.length > 0) {
      schedulesQuery.$or.push({ complementaryRequest: { $in: requestsInTownIds } });
    }

    // Defensa: si no hay ambientes ni solicitudes mapeadas para este municipio
    if (schedulesQuery.$or.length === 0) {
      return res.json({
        msg: "Reporte generado correctamente",
        total: 0,
        data: [],
    ...
    const schedules = await Schedule.find(schedulesQuery)
    ...
    const data = schedules.map((s) => ({
      horario: {
        ...
        environment: { name: s.environment?.name || "EXTERNO / SIN AMBIENTE" },
      },
  ...
  try {
    ...
    (async () => {
      try {
        ...
        const fichaNumbers = auditData.map((item) => String(item.fichaNumber).trim());
        const numericFichas = fichaNumbers.map((n) => parseInt(n, 10)).filter((n) => !isNaN(n));
        const foundRequests = await ComplementaryRequest.find({
          $or: [
            { fichaNumber: { $in: fichaNumbers } },
            { fichaNumber: { $in: numericFichas } }
          ]
        }).populate("instructor", "name email emailpersonal");
        const requestMap = new Map();
        foundRequests.forEach((r) => {
          if (r.fichaNumber) {
            requestMap.set(String(r.fichaNumber).trim(), r);
          }
        });

        for (let i = 0; i < auditData.length; i++) {
          const item = auditData[i];
          const fNumStr = String(item.fichaNumber).trim();
          // Find corresponding ComplementaryRequest (vía Map, sin query por ficha)
          const request = requestMap.get(fNumStr);
          ...
          // Proceso B: Juicios (solo si ESTADO_FICHA = "terminada por fecha" y FECHA_FIN_FICHA >= 9 días de antigüedad).
          // REGLA ESTRICTA: Fichas en ejecución NO aplican para juicios pendientes.
          const estadoNorm = (item.estado || "").toLowerCase().trim();
          const isTerminadaPorFecha = estadoNorm.includes("terminada");
          let esAntiguedadValida = false;
          ...
          if (rawFechaFin) {
            ...
            if (!isNaN(fechaFin.getTime())) {
              ...
              esAntiguedadValida = diffDays >= 9;
            }
          }

          if (isTerminadaPorFecha && esAntiguedadValida) {
        ...
        if (results.faltanJuicios.length > 0) {
          console.log(`[AUDIT-DF14] Enviando notificaciones de juicios a instructores (${results.faltanJuicios.length} fichas)...`);
          ...
          try {
            ...
            console.log(`[AUDIT-DF14] Notificaciones juicios instructores: ${notificacionJuicios.sent} enviadas, ${notificacionJuicios.failed} fallidas`);
          } catch (err) {
            console.error("[AUDIT-DF14] Error enviando notificaciones de juicios a instructores:", err.message);
          }
        }

        // SIEMPRE enviar resumen al coordinador (tenga o no juicios pendientes)
        try {
          console.log("[AUDIT-DF14] Enviando resumen de juicios al coordinador...");
          await sendCoordinatorMissingJudgmentsNotification(results.faltanJuicios);
        } catch (err) {
          console.error("[AUDIT-DF14] Error enviando notificación de juicios al coordinador:", err.message);
        }
...
compCtrl.getInstructorAllSchedules = async (req, res) => {
  try {
    ...
    const activeSchedules = schedules.filter((s) => {
      if (s.scheduleType === "COMPLEMENTARIA" && s.complementaryRequest) {
        return !["CANCELADA", "RECHAZADA", "CERRADA"].includes(s.complementaryRequest.state);
      }
      return true;
    });

    const titulada = activeSchedules.filter((s) => !s.complementaryRequest);
    const complementaria = activeSchedules.filter((s) => s.complementaryRequest);
...
compCtrl.requestExtension = async (req, res) => {
  ...
  try {
    ...
    notifyExtensionRequest(request, instructor.name || decoded.email).catch((emailErr) => {
...
compCtrl.resolveExtension = async (req, res) => {
  ...
  try {
    ...
    if (status === "APROBADA") {
      request.puedeReprogramar = true;
    }

    await request.save();

    // Notificar al instructor
    notifyExtensionResolved(request, extension).catch((emailErr) => {
    ...
    res.json({
      ...
      puedeReprogramar: status === "APROBADA",
      requestId: id,
    });
  } catch (error) {
    console.log(error);
...
// ========================= Templates de Correo de Complementarias (CRU) =========================

// GET /email-templates — Lista todos los templates con metadata (sin htmlContent completo para alivianar la respuesta)
compCtrl.getEmailTemplates = async (req, res) => {
  try {
    const templates = await ComplementaryEmailTemplate.find({ status: 0 })
      .select("key description audience variables subject defaultSubject isSegmented segments defaultSegments updatedAt")
      .sort({ audience: 1, key: 1 });
    res.json({
      msg: "Templates obtenidos con exito",
      total: templates.length,
      data: templates,
    });
  } catch (error) {
    console.log(error);
    res.status(400).json({ msg: "No fue posible obtener los templates" });
  }
};

// GET /email-templates/:key — Obtiene un template completo (con htmlContent) por su key
compCtrl.getEmailTemplateByKey = async (req, res) => {
  const { key } = req.params;
  try {
    const template = await ComplementaryEmailTemplate.findOne({ key, status: 0 });
    if (!template) {
      return res.status(404).json({ msg: `Template '${key}' no encontrado` });
    }
    const doc = template.toObject();
    doc.isSegmented = true;

    const rawTableSec = doc.segments?.tableSection || doc.defaultSegments?.tableSection || "";
    if (!/<table/i.test(rawTableSec)) {
      if (doc.segments) doc.segments.tableSection = "";
      if (doc.defaultSegments) doc.defaultSegments.tableSection = "";
    }

    res.json({
      msg: "Template obtenido con exito",
      data: doc,
    });
  } catch (error) {
    console.log(error);
    res.status(400).json({ msg: "No fue posible obtener el template" });
  }
};

function convertSegmentTextToHtml(text) {
  if (!text) return "";
  return `<div style="white-space: pre-wrap; font-family: Arial, sans-serif; font-size: 14px; line-height: 1.5; color: #333333; margin-bottom: 15px;">${text}</div>`;
}

// PUT /email-templates/:key — Actualiza subject, htmlContent o segments de un template
compCtrl.updateEmailTemplate = async (req, res) => {
  const { key } = req.params;
  const { subject, htmlContent, segments } = req.body;
  try {
    const template = await ComplementaryEmailTemplate.findOne({ key, status: 0 });
    if (!template) {
      return res.status(404).json({ msg: `Template '${key}' no encontrado` });
    ...
    template.isSegmented = true;
    
    if (segments) {
      const tableSection = template.defaultSegments?.tableSection || template.segments?.tableSection || "";
      template.segments = {
        introMessage: segments.introMessage !== undefined ? segments.introMessage : (template.segments?.introMessage || ""),
        tableSection,
        closingMessage: segments.closingMessage !== undefined ? segments.closingMessage : (template.segments?.closingMessage || "")
      };
      if (htmlContent) {
        template.htmlContent = htmlContent;
      } else {
        const introHtml = convertSegmentTextToHtml(template.segments.introMessage);
        const closingHtml = convertSegmentTextToHtml(template.segments.closingMessage);
        template.htmlContent = `<html>\n<head>\n  <style>body { font-family: Arial, sans-serif; color: #333; font-size: 14px; line-height: 1.5; }</style>\n</head>\n<body>\n${introHtml}\n${tableSection}\n${closingHtml}\n</body>\n</html>`;
      }
    ...
    await registerAction(
      "COMPLEMENTARIAS",
      { event: `ACTUALIZAR TEMPLATE EMAIL: ${key}`, data: { key } },
      req.headers.token
    );

    res.json({
      msg: "Template actualizado correctamente",
      data: {
        key: template.key,
    ...
    console.log(error);
    res.status(400).json({ msg: "No fue posible actualizar el template" });
  }
};

// POST /email-templates/:key/reset — Restaura el template a sus valores originales
compCtrl.resetEmailTemplate = async (req, res) => {
  const { key } = req.params;
  try {
    const template = await ComplementaryEmailTemplate.findOne({ key, status: 0 });
    if (!template) {
    ...
    template.htmlContent = template.defaultHtmlContent;
    if (template.isSegmented && template.defaultSegments) {
      template.segments = {
        introMessage: template.defaultSegments.introMessage || "",
        tableSection: template.defaultSegments.tableSection || "",
        closingMessage: template.defaultSegments.closingMessage || ""
      };
    }
    await template.save();
    ...
    await registerAction(
      "COMPLEMENTARIAS",
      { event: `RESTAURAR TEMPLATE EMAIL: ${key}`, data: { key } },
      req.headers.token
    );

    res.json({
      msg: "Template restaurado a los valores originales",
      data: {
        key: template.key,
        subject: template.subject,
        updatedAt: template.updatedAt,
    ...
    console.log(error);
    res.status(400).json({ msg: "No fue posible restaurar el template" });
  }
};

// ========================= Preferencias de Notificación (Coordinador/Programador) =========================

// GET /notification-prefs — Obtiene las preferencias del usuario autenticado
compCtrl.getMyNotificationPrefs = async (req, res) => {
  try {
    const decoded = webToken.getDecodedToken(req.headers.token);
    const user = await User.findById(decoded.id).select("name email role notificacionesComplementarias");
    if (!user) {
      return res.status(404).json({ msg: "Usuario no encontrado" });
    }
    res.json({
      msg: "Preferencias obtenidas con exito",
      data: {
        userId: user._id,
        name: user.name,
        email: user.email,
        role: user.role,
        notificacionesComplementarias: user.notificacionesComplementarias || { habilitado: true, emailDestino: null },
      },
    });
  } catch (error) {
    console.log(error);
    res.status(400).json({ msg: "No fue posible obtener las preferencias" });
  }
};

// PUT /notification-prefs — Actualiza las preferencias del usuario autenticado
compCtrl.updateMyNotificationPrefs = async (req, res) => {
  try {
    const decoded = webToken.getDecodedToken(req.headers.token);
    const { habilitado, emailDestino } = req.body;
    const user = await User.findById(decoded.id);
    if (!user) {
      return res.status(404).json({ msg: "Usuario no encontrado" });
    }
    if (!user.notificacionesComplementarias) {
      user.notificacionesComplementarias = { habilitado: true, emailDestino: null };
    }
    if (habilitado !== undefined) {
      user.notificacionesComplementarias.habilitado = Boolean(habilitado);
    }
    if (emailDestino !== undefined) {
      // Permitir null para limpiar el emailDestino (vuelve a usar el email del usuario)
      user.notificacionesComplementarias.emailDestino = emailDestino || null;
    }
    user.markModified("notificacionesComplementarias");
    await user.save();

    await registerAction(
      "COMPLEMENTARIAS",
      {
        event: "ACTUALIZAR PREFERENCIAS NOTIFICACION",
        data: {
          userId: user._id,
          habilitado: user.notificacionesComplementarias.habilitado,
          emailDestino: user.notificacionesComplementarias.emailDestino,
        },
      },
      req.headers.token
    );
    ...
    res.json({
      ...
      data: {
        userId: user._id,
        notificacionesComplementarias: user.notificacionesComplementarias,
      },
    });
  } catch (error) {
    console.log(error);
    res.status(400).json({ msg: "No fue posible actualizar las preferencias" });
  }
};

// ========================= Edición Administrativa Generalizada =========================
compCtrl.adminUpdateRequest = async (req, res) => {
  const { id } = req.params;
  const {
    catalogCourse,
    environment,
  ...
  try {
    ...
    // 1. Si se envía catalogCourse, verificar que exista y actualizar datos denormalizados
    if (catalogCourse) {
      const catalog = await ComplementaryCatalog.findById(catalogCourse);
      if (!catalog) {
        return res.status(400).json({ msg: "El curso del catálogo no existe" });
      }
      request.catalogCourse = catalogCourse;
      request.catalogCourseName = catalog.prfDenominacion;
      request.catalogCourseCode = String(catalog.prfCodigo);
      request.catalogCourseVersion = String(catalog.prfVersion);
      if (catalog.prfDuracionMaxima) {
        request.prfDuracionMaxima = catalog.prfDuracionMaxima;
      }
    }

    // 2. Si se envían instructores, actualizarlos en la solicitud
    if (Array.isArray(instructoresIds)) {
      const instructores = [];
      for (const instId of instructoresIds) {
        const instData = await Instructor.findById(instId);
        if (instData) {
          instructores.push({
            ...
            documento: instData.numdocument || "",
            email: instData.email || instData.emailpersonal || "",
            esPrincipal: instData._id.toString() === request.instructor.toString(),
          });
        }
      }
      if (instructores.length > 0) {
        request.instructores = instructores;
      }
    }

    // 3. Supervisor (ObjectId de User con rol COORDINADOR)
    if (supervisor !== undefined) {
      if (supervisor) {
        ...
        if (supervisorUser) {
          ...
          request.supervisorNombre = supervisorUser.name;
        }
      } else {
    ...
    // 4. Normalizar campos generales
    const normalizedFields = complementaryHelper.normalizeRequestFields(req.body);
    ...
    if (actaTrazabilidad !== undefined) {
      request.actaTrazabilidad = actaTrazabilidad ? String(actaTrazabilidad).toUpperCase().trim() : "";
    }

    // 5. Actualizar campos específicos
    if (environment !== undefined || req.body.ambienteNombre !== undefined) {
      if (environment && /^[0-9a-fA-F]{24}$/.test(environment)) {
        const envObj = await Environment.findById(environment);
        request.environment = envObj ? envObj._id : environment;
        ...
        request.ambienteNombre = req.body.ambienteNombre.trim().toUpperCase();
        request.environment = null;
      } else {
        request.environment = null;
        request.ambienteNombre = "";
    ...
    if (formationDocument !== undefined) request.formationDocument = formationDocument || "";
    if (numAprendices !== undefined) request.numAprendices = Number(numAprendices) || 0;
    if (proyectoAsociado !== undefined) request.proyectoAsociado = (proyectoAsociado || "").toUpperCase().trim();
    if (learningActivity !== undefined) request.learningActivity = (learningActivity || "").toUpperCase().trim();
    ...
    if (fichaNumber !== undefined) {
      ...
      request.fichaCaracterizacion = (fichaNumber || "").toUpperCase().trim();
    }
    if (codigoSolicitud !== undefined) request.codigoSolicitud = (codigoSolicitud || "").toUpperCase().trim();

    // 8. Competencias y resultados
    if (competencies) {
      request.competencies = complementaryHelper.normalizeCompetencies(competencies);
    }
    if (outcomes) {
      request.outcomes = outcomes.map((o) => String(o).toUpperCase().trim());
    }
    ...
    request.history.push({
      ...
      changedBy: decoded.id,
      changedByRole: decoded.rol,
      observations: "Solicitud editada de forma generalizada por administración",
    });

    await request.save();

    if (request.fichaNumber) {
      await complementaryHelper.syncFicheFromRequest(request).catch((err) => {
        console.error("Error sincronizando Fiche en adminUpdateRequest:", err);
      });
    }
    ...
    await registerAction(
      "COMPLEMENTARIAS",
      {
        event: "EDICION GENERAL ADMINISTRATIVA",
        data: { id, admin: decoded.email },
      },
      req.headers.token
    );
    ...
    res.json({
      msg: "Solicitud actualizada correctamente por el administrador",
      data: request,
    });
  } catch (error) {
    console.log(error);
...
// Obtener el próximo número consecutivo para las solicitudes de complementarias
compCtrl.getNextConsecutive = async (req, res) => {
  try {
    const count = await ComplementaryRequest.countDocuments();
    const today = new Date();
    const dateStr = `${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, "0")}${String(today.getDate()).padStart(2, "0")}`;
    const nextConsecutive = `${String(count + 1).padStart(7, "0")}-${dateStr}`;
    res.json({ nextConsecutive });
  } catch (error) {
    console.log(error);
    res.status(500).json({ msg: "No fue posible obtener el consecutivo" });
  }
};

// ==================== CRUD de Catálogo de Complementarias ====================

// Obtener catálogo para CRUD admin (trae inhabilitados por defecto)
compCtrl.getAdminCatalogs = async (req, res) => {
  const { status, prfDenominacion, prfCodigo, page, limit, q, orden } = req.query;
  try {
    const filter = {};
    if (status !== undefined && status !== "") {
      filter.status = Number(status);
    ...
    const sortOptions = ordenMap[orden] ? ordenMap[orden] : { createdAt: -1 };
    ...
    if (esModoPaginado) {
      return res.json({ data: catalogs, total, page: pageNum, limit: limitNum });
    }
...
// Crear nuevo programa en el catálogo
compCtrl.createCatalog = async (req, res) => {
  try {
    const data = req.body;
    // Si envían prfCodigo, asegurarnos de que prfCodigoStr esté en sincronía
    if (data.prfCodigo) {
      data.prfCodigoStr = String(data.prfCodigo).padStart(5, "0"); // Asumiendo mínimo 5 dígitos o similar
    }

    const newCatalog = new ComplementaryCatalog(data);
    await newCatalog.save();

    res.json({
      msg: "Programa agregado al catálogo correctamente",
      data: newCatalog,
    });
  } catch (error) {
    console.log(error);
    if (error.code === 11000) {
      return res.status(400).json({ msg: "El código del programa ya existe en el catálogo" });
    }
    res.status(400).json({ msg: "No fue posible crear el programa en el catálogo" });
  }
};

// Actualizar programa existente
compCtrl.updateCatalog = async (req, res) => {
  try {
    const { id } = req.params;
    const data = req.body;

    if (data.prfCodigo) {
      data.prfCodigoStr = String(data.prfCodigo).padStart(5, "0");
    }

    const updatedCatalog = await ComplementaryCatalog.findByIdAndUpdate(id, data, {
      new: true,
      runValidators: true,
    });
    ...
    res.status(400).json({ msg: "No fue posible actualizar el programa en el catálogo" });
  }
};

// Habilitar/Inhabilitar programa (Toggle status)
compCtrl.toggleCatalogStatus = async (req, res) => {
  try {
    const { id } = req.params;
    const catalog = await ComplementaryCatalog.findById(id);

    // Cambiamos de activo a inactivo y viceversa (0 = activo, 1 = inactivo)
    const newStatus = catalog.status === 0 ? 1 : 0;
    
    catalog.status = newStatus;
    await catalog.save();

    const statusMsg = newStatus === 0 ? "habilitado" : "inhabilitado";
    res.json({
      msg: `Programa ${statusMsg} correctamente`,
      data: catalog,
    });
  } catch (error) {
    console.log(error);
    res.status(400).json({ msg: "No fue posible cambiar el estado del programa" });
  }
};

export async function getDf14aCronStatusController(req, res) {
  try {
    const status = getDf14aCronStatus();
    res.json(status);
  } catch (error) {
    res.status(500).json({ msg: "Error al obtener estado del cron DF-14A", error: error.message });
  }
}

export async function toggleDf14aCronController(req, res) {
  try {
    const { enabled } = req.body;
    if (enabled === true) {
      startDf14aCron();
      res.json({ enabled: true, msg: "Cron semanal DF-14A ACTIVADO correctamente" });
    } else {
      stopDf14aCron();
      res.json({ enabled: false, msg: "Cron semanal DF-14A DESACTIVADO correctamente" });
    }
  } catch (error) {
    res.status(500).json({ msg: "Error al cambiar estado del cron DF-14A", error: error.message });
  }
}
