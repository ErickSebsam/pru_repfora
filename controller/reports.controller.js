...
reportsCtrl.responseInstr = async (req, res) => {  
  ...
  try {
    ...
    if (!instructor.email && instructor.emailpersonal) {
      instructor.email = instructor.emailpersonal;
    }
    ...
    const instEmail = (instructor.email || "").toLowerCase();
    const instPersonalEmail = (instructor.emailpersonal || "").toLowerCase();

    if (instEmail === userEmail || instPersonalEmail === userEmail) {
...
reportsCtrl.newReport = async (req, res) => {
  ...
  try {
    const schedules = await Schedule.find({ fiche: fiche, status: 0 }).populate(
      "fiche instructor program competence outcome environment complementaryRequest"
    );
    ...
    return res.json({
      ...
      fiche: schedules[0] ? (schedules[0].fiche?.number || schedules[0].complementaryRequest?.fichaNumber || schedules[0].complementaryRequest?.fichaCaracterizacion || "") : "",
      program: schedules[0] ? (schedules[0].program?.name || schedules[0].complementaryRequest?.catalogCourseName || "") : "",
      months: dates,
      yearsMonth,
      events: daysProgram,
    });
...
reportsCtrl.newReportInstructor = async (req, res) => {
  ...
  try {
    const schedules = await Schedule.find({
      ...
      status: 0,
    }).populate("fiche instructor program competence outcome environment complementaryRequest");
    ...
    //extraer los eventos de cada programacion y crear un objeto con id, idInstructor, title, start
    newListProgramaFiche.forEach((progra) => {
      const ficheNum = progra.fiche?.number || progra.complementaryRequest?.fichaNumber || progra.complementaryRequest?.fichaCaracterizacion || "SIN_FICHA";
      let color;
      if (colorsFiche[ficheNum]) {
        color = colorsFiche[ficheNum];
      } else {
        if (colors.length === 0) colors = [...colorsEvents];
        color = colors[Math.floor(Math.random() * colors.length)] || { background: "#4e73df", colortext: "#ffffff" };
        const idxColor = colors.indexOf(color);
        if (idxColor !== -1) colors.splice(idxColor, 1);
        colorsFiche[ficheNum] = color;
      }
    ...
    newListOtherSchedule.forEach((progra) => {
      let color = colors[Math.floor(Math.random() * colors.length)] || { background: "#4e73df", colortext: "#ffffff" };
      const idxColor = colors.indexOf(color);
      if (idxColor !== -1) colors.splice(idxColor, 1);
        ...
        if (dates.includes(month)) {
          if (daysProgram[month]) {
            daysProgram[month].push({
              ...
              instructor: progra.instructor?.name || "SIN ASIGNAR",
...
reportsCtrl.newReportEnvironment = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      msg: "Reporte generado correctamente",
      environment: schedules[0] ? (schedules[0].environment?.name || schedules[0].complementaryRequest?.ambienteNombre || "") : "",
      months: dates,
      yearsMonth,
      events: daysProgram,
    });
...
reportsCtrl.reportHoursInstructors = async (req, res) => {
  ...
  try {
    ...
    // Normalizar fstart/fend a medianoche UTC para que la comparacion
    // de eventos coincida con calculateHours (reporte por instructor)
    const newFstart = dateFormater(fstart);
    const newFend = dateFormater(fend);

    if (type == "EN FORMACION" || type == "TODOS") {
      // Filtrar por traslape de rangos: coincide con newReportInstructor y
      // evita perder schedules cuyo fstart es antiguo pero cuyos eventos
      // caen dentro del rango pedido.
      const schedules = await Schedule.find({
        fstart: { $lte: newFend },
        fend: { $gte: newFstart },
        status: 0,
    ...
    // Redondear el total final por instructor y por mes (la suma interna
    // mantiene precision con toFixed(2), solo se redondea al presentar)
    groupInstructor.forEach((instructor) => {
      instructor.hours = Math.round(instructor.hours);
      instructor.months.forEach((m) => {
        m.hours = Math.round(m.hours);
      });
    });
