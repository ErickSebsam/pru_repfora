...
scheduleCtrl.getScheduleId = async (req, res) => {
  ...
  try {
    ...
    if (!schedule) {
      return res.status(404).json({ msg: "El horario no existe" });
    }
    ...
    const totalEventsCount = (newSchedule.events || []).length || 1;

    (newSchedule.events || []).forEach((event, eventIdx) => {
      if (!event) return;
      const eventDate = typeof event === 'string' ? event.split("T")[0] : new Date(event).toISOString().split("T")[0];
      
      let outcomeText = schedule.outcome?.outcomes || "";
      let competenceText = schedule.competence?.name || "";

      if ((schedule.scheduleType === "COMPLEMENTARIA" || schedule.complementaryRequest) && !outcomeText) {
        const compReq = schedule.complementaryRequest;
        if (compReq) {
          const session = compReq.sesiones?.find(s => s.fecha === eventDate);
          const sessionRes = session?.resultados?.filter(Boolean) || [];
          if (sessionRes.length > 0) {
            outcomeText = sessionRes.join(", ");
          } else if (compReq.outcomes?.length > 0) {
            const validOutcomes = compReq.outcomes.filter(Boolean);
            if (validOutcomes.length > 0) {
              const outcomeIdx = Math.min(
                Math.floor((eventIdx * validOutcomes.length) / totalEventsCount),
                validOutcomes.length - 1
              );
              outcomeText = validOutcomes[outcomeIdx];
            }
          }

          if (session && session.competencia) {
            competenceText = session.competencia;
          } else if (compReq.competencies?.length) {
            competenceText = compReq.competencies.map(c => c.nombre || c.codigo).filter(Boolean).join(", ");
          }
        }
      }
      ...
      newListEvents.push({
        id: index,
        idInstructor: schedule.instructor?._id || null,
        fiche: schedule.fiche?.number || schedule.complementaryRequest?.fichaNumber || "",
        program: schedule.program?.name || schedule.complementaryRequest?.catalogCourseName || "",
        environment: schedule.environment?.name || "",
        outcome: sanitizeOutcome(outcomeText),
        competence: sanitizeCompetence(competenceText),
