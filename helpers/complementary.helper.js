...
/**
 * Sincroniza o crea el documento en la colección Fiche correspondiente a una ComplementaryRequest.
 * @async
 * @function syncFicheFromRequest
 * @memberof complementaryHelper
 * @param {Object} request - Objeto ComplementaryRequest (o documento mongoose)
 * @param {Object} [options={}] - Opciones adicionales (ej. fichaNumber, fechaInicio, fechaFin, status)
 * @returns {Promise<Object|null>} Documento Fiche sincronizado o null si no tiene fichaNumber
 */
complementaryHelper.syncFicheFromRequest = async (request, options = {}) => {
  if (!request) return null;

  const rawNumber =
    options.fichaNumber ||
    request.fichaNumber ||
    request.fichaCaracterizacion;

  if (!rawNumber || String(rawNumber).trim() === "") {
    return null;
  }
  const normalizedNumber = String(rawNumber).trim().toUpperCase();

  // 1. Obtener coordinación "PROGRAMAS ESPECIALES"
  const coord = await coordinationHelper.findCoordinationByName(
    "PROGRAMAS ESPECIALES"
  );
  if (!coord) {
    throw new Error("No existe la coordinación PROGRAMAS ESPECIALES");
  }

  // 2. Buscar o crear Program
  let dbProgram = null;
  if (request.catalogCourseCode && request.catalogCourseName) {
    dbProgram = await Program.findOne({
  ...
  // 3. Determinar instructor principal
  const instructorId =
    request.instructor ||
    (request.instructores && request.instructores.length > 0
      ? (request.instructores.find((i) => i.esPrincipal)?.instructor ||
         request.instructores[0]?.instructor)
      : null);

  // 4. Fechas de la ficha
  const fstart = options.fechaInicio
    ? new Date(options.fechaInicio)
    : request.fechaInicio
    ? new Date(request.fechaInicio)
    : new Date();
  const fend = options.fechaFin
    ? new Date(options.fechaFin)
    : request.fechaFin
    ? new Date(request.fechaFin)
    : new Date();

  // 5. Buscar si ya existe la ficha por complementaryRequest o por number
  ...
  if (fiche) {
    // Si la ficha existente pertenecía a otra solicitud complementaria diferente
    if (
      fiche.complementaryRequest &&
      String(fiche.complementaryRequest) !== String(request._id)
    ) {
      throw new Error(
        `El número de ficha ${normalizedNumber} ya está asociado a otra solicitud complementaria`
      );
    }

    fiche.number = normalizedNumber;
    if (dbProgram) fiche.program = dbProgram._id;
    if (instructorId) fiche.owner = instructorId;
    fiche.coordination = coord._id;
    fiche.fstart = fstart;
    fiche.fend = fend;
    fiche.type = "COMPLEMENTARIA";
    fiche.complementaryRequest = request._id;
    if (options.status !== undefined) {
      fiche.status = options.status;
    }
    await fiche.save();
  } else {
    fiche = new Fiche({
      ...
      fstart,
      fend,
      status: options.status !== undefined ? options.status : 0,
      type: "COMPLEMENTARIA",
      complementaryRequest: request._id,
    });
    await fiche.save();
...
complementaryHelper.validateRequestApprovedOrProgrammed = async (id) => {
  try {
    const request = await ComplementaryRequest.findById(id, { status: 0 });
    if (!request) throw new Error();
    if (!["APROBADA", "PROGRAMADA"].includes(request.state)) throw new Error();
  } catch (error) {
    throw new Error("La solicitud no existe o no está en estado APROBADA o PROGRAMADA");
  }
};
...
complementaryHelper.validateStateTransition = async (id, newState) => {
  const validTransitions = {
    ...
    APROBADA: ["CODIGO_ASIGNADO", "FICHA_ASIGNADA", "INSCRIPCION", "PROGRAMADA", "CANCELADA"],
    PROGRAMADA: ["CODIGO_ASIGNADO", "FICHA_ASIGNADA", "INSCRIPCION", "MATRICULADA", "EJECUCION", "CANCELADA"],
    CODIGO_ASIGNADO: ["INSCRIPCION", "MATRICULADA", "CANCELADA"],
    FICHA_ASIGNADA: ["INSCRIPCION", "MATRICULADA", "CANCELADA"],
    INSCRIPCION: ["MATRICULADA", "EJECUCION", "CANCELADA"],
    MATRICULADA: ["EJECUCION", "CANCELADA"],
    EJECUCION: ["CERRADA", "CANCELADA"],
  };
  try {
    const request = await ComplementaryRequest.findById(id, { status: 0 });
    if (!request) throw new Error();
    if (request.state === newState) return; // Ya está en ese estado (no-op para evitar errores en llamadas redundantes de frontend)
...
complementaryHelper.validateFormationDataEditable = async (id) => {
  try {
    ...
    if (request.state !== "APROBADA" && request.state !== "CODIGO_ASIGNADO" && request.state !== "FICHA_ASIGNADA") {
      throw new Error("La solicitud debe estar en estado APROBADA o CODIGO_ASIGNADO para agregar datos de formación");
    }
...
complementaryScheduleHelper.validateRequestProgrammable = async (requestId) => {
  ...
  if (!validStates.includes(request.state)) {
    throw new Error(
      `La solicitud debe estar en estado APROBADA, CODIGO_ASIGNADO, FICHA_ASIGNADA, INSCRIPCION, MATRICULADA, PROGRAMADA o EJECUCION para poder programarla. Estado actual: ${request.state}`
    );
  }
  return request;
};
...
complementaryScheduleHelper.validateInstructorAvailability = async (
  ...
  const schedules = await Schedule.find(query).populate(
    "fiche instructor environment complementaryRequest"
  );
  ...
  for (const schedule of schedules) {
    if (
      schedule.scheduleType === "COMPLEMENTARIA" &&
      schedule.complementaryRequest &&
      ["CANCELADA", "RECHAZADA", "CERRADA"].includes(schedule.complementaryRequest.state)
    ) {
      continue;
    }
...
function _splitResultados(seg) {
  ...
  return parts.map((s) => sanitizeOutcome(s.toUpperCase()));
}
...
function _parseCompetenceBlock(block) {
  try {
    ...
    if (!code) return null;
    const name = sanitizeCompetence(_norm(head.replace(/\b\d{9}\b/g, " ")).toUpperCase());
