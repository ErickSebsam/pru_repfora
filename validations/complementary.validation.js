...
const {
  ...
  validateRequestApprovedOrProgrammed,
...
complementaryVali.validateAdminCatalogQuery = [
  check("token").custom(async (token) => {
    await validateToken(token, false);
    const decoded = jwt.decode(token);
    if (!decoded || !["COORDINADOR", "PROGRAMADOR"].includes(decoded.rol)) {
      throw new Error("No tienes permisos para consultar el CRUD del catálogo.");
    }
  }),
  check("page").optional().isInt({ min: 1 }).withMessage("page debe ser un entero mayor o igual a 1"),
  check("limit").optional().isInt({ min: 1 }).withMessage("limit debe ser un entero mayor o igual a 1"),
...
complementaryVali.validateCreateCatalog = [
  check("token").custom(async (token) => {
    // Validamos la firma y usuario
    await validateToken(token, false);
    // Verificamos que sea estrictamente coordinador o programador
    const decoded = jwt.decode(token);
    if (!decoded || !["COORDINADOR", "PROGRAMADOR"].includes(decoded.rol)) {
      throw new Error("No tienes permisos para realizar esta acción. Solo Coordinadores y Programadores pueden gestionar el catálogo.");
    }
  }),
  check("prfCodigo", "El código del programa es obligatorio").notEmpty(),
  check("prfCodigo", "El código del programa debe ser numérico").isNumeric(),
  check("prfVersion", "La versión del programa es obligatoria").notEmpty(),
  check("prfVersion", "La versión debe ser numérica").isNumeric(),
  check("codVer", "El código versión es obligatorio").notEmpty(),
  check("prfDenominacion", "El nombre del programa es obligatorio").notEmpty(),
  validateFields,
];

complementaryVali.validateUpdateCatalog = [
  check("token").custom(async (token) => {
    await validateToken(token, false);
    const decoded = jwt.decode(token);
    if (!decoded || !["COORDINADOR", "PROGRAMADOR"].includes(decoded.rol)) {
...
complementaryVali.validateToggleCatalogStatus = [
  check("token").custom(async (token) => {
    await validateToken(token, false);
    const decoded = jwt.decode(token);
    if (!decoded || !["COORDINADOR", "PROGRAMADOR"].includes(decoded.rol)) {
...
complementaryVali.validateRegisterRequest = [
  ...
  check("environment").optional({ checkFalsy: true }).isMongoId().withMessage("El ambiente debe ser un ID de Mongoose válido"),
  ...
  // RF-03: tipoPrograma es OBLIGATORIO al registrar la solicitud.
  // tipoPoblacion es opcional. Si se envían, deben existir en el catálogo
  ...
  check("tipoPoblacion").optional().isString().withMessage("El tipo de población debe ser un texto"),
  check("tipoPoblacion")
    .optional()
  ...
  check("catalogCourse").custom((_, { req }) => {
    if (!req.files || !req.files.solicitudPdf || !req.files.cedulas || !req.files.cedulasPdf) {
      throw new Error("Debe adjuntar los 3 archivos obligatorios: PDF de solicitud (solicitudPdf), archivo de cédulas en Excel (cedulas) y PDF de cédulas (cedulasPdf)");
    }

    const { solicitudPdf, cedulas, cedulasPdf } = req.files;

    if (solicitudPdf.mimetype !== "application/pdf") {
      throw new Error("El archivo de solicitud (solicitudPdf) debe ser de tipo PDF");
    }

    if (cedulasPdf.mimetype !== "application/pdf") {
      throw new Error("El archivo de cédulas en PDF (cedulasPdf) debe ser de tipo PDF");
    }
    ...
    const ext = cedulas.name ? cedulas.name.split(".").pop().toLowerCase() : "";
    if (!excelMimeTypes.includes(cedulas.mimetype) && ext !== "xls" && ext !== "xlsx") {
      throw new Error("El archivo de cédulas (cedulas) debe ser un archivo de Excel válido (.xls o .xlsx)");
    }
  ...
  check("token").custom(async (token, { req }) => {
    ...
    if (decoded.isInstructor) {
      ...
      if (!["ADMIN", "COORDINADOR", "PROGRAMADOR"].includes(decoded.rol)) {
        throw new Error("Solo un administrador, coordinador o programador puede realizar esta acción");
      }
    }
    return true;
  }),
  check("id").custom(async (id, { req }) => {
    if (req.decodedToken && !req.decodedToken.isInstructor) {
      const request = await ComplementaryRequest.findById(id);
      if (!request) {
        throw new Error("La solicitud no existe");
      }
      if (["CERRADA", "CANCELADA"].includes(request.state)) {
        throw new Error(`No se puede modificar una solicitud en estado ${request.state}`);
      }
    } else {
...
complementaryVali.validateAssignFicha = [
  ...
  check("id").custom(async (id) => {
    ...
    if (["CERRADA", "CANCELADA"].includes(request.state)) {
      throw new Error(`No se puede gestionar ficha en estado ${request.state}`);
    }
  }),
  check("fichaCaracterizacion").optional({ checkFalsy: true }).custom(async (fichaCaracterizacion) => {
    await ficheHelper.uniqueNumberFiche(String(fichaCaracterizacion).toUpperCase().trim());
  }),
  check("fichaNumber").optional({ checkFalsy: true }).custom(async (fichaNumber) => {
    await ficheHelper.uniqueNumberFiche(String(fichaNumber).toUpperCase().trim());
  }),
  check("codigoSolicitud").optional({ checkFalsy: true }).isString().trim(),
  check("proyectoAsociado").optional({ checkFalsy: true }).isString().trim(),
  check("actaTrazabilidad").optional({ checkFalsy: true }).isString().trim(),
  check("fechaInicio", "La fecha de inicio no es valida").optional({ checkFalsy: true }).isISO8601(),
  check("fechaFin", "La fecha de finalización no es valida").optional({ checkFalsy: true }).isISO8601(),
  check("fechaInscripcionInicio", "La fecha de inicio de inscripción no es válida").optional({ checkFalsy: true }).isISO8601(),
  check("fechaInscripcionFin", "La fecha de fin de inscripción no es válida").optional({ checkFalsy: true }).isISO8601(),
  check("fechaMatriculaInicio", "La fecha de inicio de matrícula no es valida").optional({ checkFalsy: true }).isISO8601(),
  check("fechaMatriculaFin", "La fecha de fin de matrícula no es valida").optional({ checkFalsy: true }).isISO8601(),
  check("environment").optional({ checkFalsy: true }).isMongoId().withMessage("El ambiente no es válido"),
  check("ambienteNombre").optional({ checkFalsy: true }).isString().trim(),
  ...
  check("fechaInscripcionInicio").custom(async (inicio, { req }) => {
    if (inicio && req.body.fechaInscripcionFin) {
      if (new Date(inicio) > new Date(req.body.fechaInscripcionFin)) {
        throw new Error("La fecha de inicio de inscripción no puede ser mayor a la de fin");
      }
    }
  }),
  check("token").custom(async (token) => {
...
complementaryVali.validateChangeState = [
  ...
  check("token").custom(async (token) => {
    ...
    if (
      ...
      throw new Error("Solo un administrador, coordinador o programador puede cambiar el estado");
    }
  }),
  validateFields,
];
...
complementaryVali.validateScheduleComplementary = [
  ...
  check("events").optional().isArray().withMessage("Los eventos deben ser un array"),
  check("events").custom((events) => {
  ...
  check("id").custom((id, { req }) => {
    if (!req.body.events && !req.body.sesiones) {
      throw new Error("Los eventos o las sesiones son obligatorios");
    }
    return true;
  }),
  check("token").custom(async (token) => {
    ...
    if (
      ...
      throw new Error("Solo un administrador, coordinador o programador puede programar horarios complementarios");
    }
  }),
  validateFields,
];
...
complementaryVali.validateReschedule = [
  ...
  check("token").custom(async (token, { req }) => {
    const decoded = jwt.decode(token);
    // Token normal de admin/coordinador/programador
    if (decoded?.scope !== "VERIFY") {
      await validateToken(token);
      if (!["ADMIN", "COORDINADOR", "PROGRAMADOR"].includes(decoded?.rol)) {
    ...
    // Token complementaria (instructor): solo permitido si la solicitud tiene puedeReprogramar activo
    await validateTokenComplementaria(token);
    // Buscar el schedule por el id del param para llegar al request asociado
    const schedule = await (await import("../models/Schedule.js")).default.findById(req.params.id);
    if (!schedule || !schedule.complementaryRequest) {
      throw new Error("Horario no encontrado");
    }
    const ComplementaryRequest = (await import("../models/ComplementaryRequest.js")).default;
    const request = await ComplementaryRequest.findById(schedule.complementaryRequest);
    if (!request || !request.puedeReprogramar) {
      throw new Error("No tienes permiso para reprogramar esta ficha. Solicita una ampliacion primero.");
    }
  }),
...
const rolesPermitidosParametro = ["ADMIN" , "COORDINADOR", "PROGRAMADOR"];
...
// Roles restringidos a ADMIN, COORDINADOR y PROGRAMADOR: el endpoint corre Playwright y usa
...
complementaryVali.validateAuditDF14Test = [
  check("token").custom(async (token) => {
    ...
    const decoded = jwt.decode(token);
    if (!["ADMIN", "COORDINADOR", "PROGRAMADOR"].includes(decoded?.rol)) {
      throw new Error("Solo un administrador, coordinador o programador puede ejecutar el reporte DF-14A");
    }
  }),
  validateFields,
];
...
// ========================= Validaciones: Templates de Correo =========================

// GET /email-templates — Lista todos los templates
// Roles: COORDINADOR, PROGRAMADOR
complementaryVali.validateGetEmailTemplates = [
  check("token").custom(async (token) => {
    await validateToken(token);
    const decoded = jwt.decode(token);
    if (!["COORDINADOR", "PROGRAMADOR"].includes(decoded?.rol)) {
      throw new Error("Solo coordinadores o programadores pueden gestionar los templates de correo");
    }
    return true;
  }),
  validateFields,
...
// GET /email-templates/:key — Obtiene un template por key
// Roles: COORDINADOR, PROGRAMADOR
complementaryVali.validateGetEmailTemplateByKey = [
  check("key", "La clave del template es obligatoria").notEmpty().isString(),
  check("token").custom(async (token) => {
    await validateToken(token);
    const decoded = jwt.decode(token);
    if (!["COORDINADOR", "PROGRAMADOR"].includes(decoded?.rol)) {
...
// PUT /email-templates/:key — Actualiza subject y/o htmlContent de un template
// Roles: COORDINADOR, PROGRAMADOR
complementaryVali.validateUpdateEmailTemplate = [
  check("key", "La clave del template es obligatoria").notEmpty().isString(),
  check("subject")
    .optional()
  ...
  check("htmlContent")
    .optional()
    .isString().withMessage("El contenido HTML debe ser un texto")
    .notEmpty().withMessage("El contenido HTML no puede estar vacío"),
  check("token").custom(async (token) => {
    await validateToken(token);
    const decoded = jwt.decode(token);
    if (!["COORDINADOR", "PROGRAMADOR"].includes(decoded?.rol)) {
      throw new Error("Solo coordinadores o programadores pueden editar los templates de correo");
    }
    return true;
  }),
  validateFields,
...
// Roles: COORDINADOR, PROGRAMADOR
complementaryVali.validateResetEmailTemplate = [
  check("key", "La clave del template es obligatoria").notEmpty().isString(),
  check("token").custom(async (token) => {
    await validateToken(token);
    const decoded = jwt.decode(token);
    if (!["COORDINADOR", "PROGRAMADOR"].includes(decoded?.rol)) {
      throw new Error("Solo coordinadores o programadores pueden restaurar los templates de correo");
    }
    return true;
  }),
  validateFields,
...
// ========================= Validaciones: Preferencias de Notificación =========================

// GET /notification-prefs — Lee las preferencias del usuario autenticado
// Roles: COORDINADOR, PROGRAMADOR
complementaryVali.validateGetNotificationPrefs = [
  check("token").custom(async (token) => {
    await validateToken(token);
    const decoded = jwt.decode(token);
    if (!["COORDINADOR", "PROGRAMADOR"].includes(decoded?.rol)) {
      throw new Error("Solo coordinadores o programadores pueden gestionar preferencias de notificación");
    }
    return true;
  }),
  validateFields,
...
// Roles: COORDINADOR, PROGRAMADOR
complementaryVali.validateUpdateNotificationPrefs = [
  check("habilitado")
    .optional()
    .isBoolean().withMessage("El campo 'habilitado' debe ser un booleano (true/false)"),
  check("emailDestino")
    .optional({ nullable: true })
    .custom((value) => {
      if (value === null || value === "" || value === undefined) return true;
      // Permite uno o múltiples correos separados por comas o punto y coma
      ...
      for (const email of emails) {
        if (!emailRegex.test(email)) {
          throw new Error(
            "Cada dirección en emailDestino debe ser un email válido o null para usar el email por defecto (puedes separar múltiples correos por coma)"
          );
        }
      }
      return true;
...
// ========================= Validaciones: Edición Administrativa Generalizada =========================
complementaryVali.validateAdminUpdateRequest = [
  check("id", "El id es obligatorio").notEmpty(),
  check("id", "El id no es valido").isMongoId(),
  check("id").custom(async (id) => {
    await validateExistRequestById(id);
  ...
  check("catalogCourse").optional({ checkFalsy: true }).isMongoId().withMessage("El curso del catálogo debe ser un ID válido"),
  check("instructores").optional({ checkFalsy: true }).isArray().withMessage("Los instructores deben ser un array"),
  check("instructores.*").optional({ checkFalsy: true }).isMongoId().withMessage("Cada instructor debe ser un ID válido"),
  check("instructores").custom(async (instructores) => {
    if (!Array.isArray(instructores)) return true;
    for (const instId of instructores) {
      if (instId) await complementaryHelper.findInstructorById(instId);
    }
    return true;
  }),
  check("supervisor").optional({ checkFalsy: true }).isMongoId().withMessage("El supervisor debe ser un ID válido"),
  check("environment").optional({ checkFalsy: true }).isMongoId().withMessage("El ambiente debe ser un ID de Mongoose válido"),
  check("fechaInicio").optional({ checkFalsy: true }).isISO8601().withMessage("La fecha de inicio no es válida"),
  check("fechaFin").optional({ checkFalsy: true }).isISO8601().withMessage("La fecha de fin no es válida"),
  check("fechaInscripcionInicio").optional({ checkFalsy: true }).isISO8601().withMessage("La fecha de inicio de inscripción no es válida"),
  check("fechaInscripcionFin").optional({ checkFalsy: true }).isISO8601().withMessage("La fecha de fin de inscripción no es válida"),
  check("fechaMatriculaInicio").optional({ checkFalsy: true }).isISO8601().withMessage("La fecha de inicio de matrícula no es válida"),
  check("fechaMatriculaFin").optional({ checkFalsy: true }).isISO8601().withMessage("La fecha de fin de matrícula no es válida"),
  check("actaTrazabilidad").optional({ checkFalsy: true }).isString().trim(),
  check("tipoPrograma")
    .optional({ checkFalsy: true })
    .custom(async (tipoPrograma) => {
