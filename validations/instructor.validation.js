...
instructorVali.validateRegisterInstr = [
  ...
  check("emailpersonal", "El email personal no es valido").optional({ checkFalsy: true }).isEmail(),
  check("emailpersonal").optional({ checkFalsy: true }).custom(async (emailpersonal) => {
    await validateEmailPersonalUnique(emailpersonal);
  }),

  check("email", "El email no es valido").optional({ checkFalsy: true }).isEmail(),
  check("email").optional({ checkFalsy: true }).custom(async (email) => {
    await validateEmailUnique(email);
  }),
  check("email").custom(async (email, { req }) => {
    if (!email && !req.body.emailpersonal) {
      throw new Error("Debe proporcionar al menos un correo (institucional o personal)");
    }
  }),
...
instructorVali.validateUpdateInstr = [
  ...
  check("emailpersonal", "El email personal no es valido").optional({ checkFalsy: true }).isEmail(),
  check("emailpersonal").optional({ checkFalsy: true }).custom(async (emailpersonal, { req }) => {
    await validateEmailPersonalUnique(emailpersonal, req.params.id);
  }),

  check("email", "El email no es valido").optional({ checkFalsy: true }).isEmail(),
  check("email").optional({ checkFalsy: true }).custom(async (email, { req }) => {
